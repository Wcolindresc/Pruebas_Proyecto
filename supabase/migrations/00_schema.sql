...
-- Enum product_status (idempotente)
do $$
begin
  if not exists (select 1 from pg_type t where t.typname = 'product_status') then
...
-- Índices con nombre e IF NOT EXISTS
...
select id, product_id, public_url, sort_order, is_primary
from product_images;

-- ============== Mejoras de esquema/stock ==============
...
-- (Re)crear la constraint de stock no negativo de forma idempotente
alter table if exists product_variants
  drop constraint if exists ck_stock_nonneg;

alter table if exists product_variants
  add constraint ck_stock_nonneg check (stock >= 0);

-- Trigger para descontar stock cuando la orden pasa a 'pagado'
